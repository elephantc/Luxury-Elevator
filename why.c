...
void history_message(int from, int to, int c,int waitnum){
	...
	for(int i=0;i<SECONDLINE;i++)
	{
	 ...
	 if(from==21){
                        history[num][0][0]=c+'0';
                        if(c<2) strcpy(&history[num][0][1],"번ELEVATOR(저층용) ");
                        else if(c<4) strcpy(&history[num][0][1],"번ELEVATOR(고층용) ");
                        else if(c<6) strcpy(&history[num][0][1],"번ELEVATOR(전층용) ");

                        history[num][1][count++]=to;
                        strcpy(&history[num][1][count],"층취소           ");
        }else{
	...
	history[num][1][0]=c+'0';
        ...
        else if(c<4) strcpy(&history[num][1][1],"번ELEVATOR(고층용)응답        ");
        else if(c<6) strcpy(&history[num][1][1],"번ELEVATOR(전층용)응답        ");
        else {
	...
	while(1){
		...
		if(QisEmpty()==0){//대기하던 요청들꺼내기
			...
			if(temp<EV_MAX&&temp>=0)
			{
				number++;
				number=number%10;
				history_message(deq.from,deq.to,temp,number);
			}
			//EV를 배정받은경우 출력
		}
